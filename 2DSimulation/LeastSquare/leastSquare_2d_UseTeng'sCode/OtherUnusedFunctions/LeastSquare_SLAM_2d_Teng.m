function LeastSquare_EstimationResult = LeastSquare_SLAM_2d_Teng(data)
%% Load pre-given data: odometry and observations
if nargin < 1
%     load('../../EKF-SLAM-SmallExample/2d_datagen/data_2d.mat');   % two dot means the previous folder, while one dot means the current folder
    %load('data_2d.mat')
    load('../../2d_datagen/data_2d/data_2d_1.mat');
end
...
% load('../../EKF-SLAM-SmallExample/2d_datagen/NoiseDataInPose0Frame_2d.mat');
% load('GroundNoiseData_2d.mat');
InitialPosition = NoiseDataInPose0Frame.poses.position;
InitialRotation = NoiseDataInPose0Frame.poses.orientation;
InitialLandmark = NoiseDataInPose0Frame.landmarks;
...
for i=1:num_land
    ...
    E_li = Graph.Nodes.(NodeTypeArray{2}).Values.(['landmark' num2str(i)]);
