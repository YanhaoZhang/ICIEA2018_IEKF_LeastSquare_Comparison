...
%     load('../../2d_datagen/data_2d_InitialGuess/NoiseDataInPose0Frame_2d_1.mat');
    ...
    load('./NoiseDataInPose0Frame_2d_1.mat');
...
if ~isempty(AllOdometry)
    num_odom = size(AllOdometry,1) / 3; % number of odometry
    for i = 1:num_odom
        OdometryFromThis2Next = AllOdometry(3*i-2:3*i,:);
...
if ~isempty(AllObservation)
    num_obsv = size(AllObservation,1) / 2;   % number of all observation
    for i = 1:num_obsv
        CurrentObservation = AllObservation(2*i-1:2*i,:);
...
% debug = full(Graph.cov);full (poseCov);
